...
async function loadDownloadCount() {
  ...
  if (cached) setDownloadDisplay(cached.count);
  ...
  try {
    ...
    if (res.status === 403) {
      ...
      if (reset)
        console.warn(
          `Rate limit resets at ${new Date(reset * 1000).toLocaleTimeString()}`,
        );
      if (!cached) setDownloadDisplay("—");
      return;
    }
...
function animateSnake() {
  ...
  while (path.length < maxLength) {
    ...
    if (
      isHorizontal &&
      lineGrid.horizontal[row] &&
      lineGrid.horizontal[row][col]
    ) {
      path.push(lineGrid.horizontal[row][col]);
    } else if (
      !isHorizontal &&
      lineGrid.vertical[col] &&
      lineGrid.vertical[col][row]
    ) {
    ...
    if (isHorizontal) {
      if (lineGrid.vertical[col + 1]) {
        if (row > 0)
          directions.push({ isHorizontal: false, row: row - 1, col: col + 1 });
        if (row < rows - 1)
          directions.push({ isHorizontal: false, row: row, col: col + 1 });
      }
      ...
      if (lineGrid.horizontal[row + 1]) {
        if (col > 0)
          directions.push({ isHorizontal: true, row: row + 1, col: col - 1 });
        if (col < cols - 1)
          directions.push({ isHorizontal: true, row: row + 1, col: col });
      }
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        ...
        updateProgressDots(entry.target.getAttribute("data-section"));
      }
    });
  }, observerOptions);
  ...
  document.querySelectorAll(".progress-dot").forEach((dot) => {
    dot.addEventListener("click", () => {
      const section = document.querySelector(
        `[data-section="${dot.getAttribute("data-section")}"]`,
      );
...
function initSlider() {
  ...
  if (!heroImages.length || !heroImageContainer) return;
